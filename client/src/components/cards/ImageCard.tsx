...
export default function FlipCard({
  ...
  return (
    <div
      className="flip-perspective relative aspect-[3/4] group"
      onClick={() => setFlipped((f) => !f)}
    >
      <div
        className={`flip-inner relative w-full h-full ${flipped ? "is-flipped" : ""}`}
      >
        {/* Front */}
        <div className="flip-face paper-card !p-0">
          ...
          <div className="absolute top-2 right-2 px-2 py-0.5 rounded-full bg-black/45 text-white text-[10px] backdrop-blur-md">
            {image.terms.length} 词
          </div>
          <div className="absolute bottom-2 left-2 right-2">
            <div className="px-2 py-1 rounded-md card-filename-label text-[10px] font-medium truncate max-w-[70%] backdrop-blur-sm">
              {image.originalName}
            </div>
          </div>
        </div>
          ...
          <div className="absolute inset-[2px] rounded-[12px] bg-card p-4 flex flex-col overflow-hidden">
            ...
            <div className="relative flex-1 overflow-y-auto flex flex-wrap gap-1.5 content-start pr-1">
              {image.terms.length === 0 && (
                <p className="text-xs text-muted-foreground font-hand text-lg">
                  还没解析出术语～
                </p>
              )}
              {image.terms.map((t, i) => (
                <button
                  ...
                  className="term-sticker animate-wiggle-in group/term relative"
                  style={{ animationDelay: `${i * 40}ms` }}
                >
                  {copied === t.term ? (
                    <>
                      <Check className="w-3 h-3 mr-1 text-primary" /> 已复制
                    </>
                  ) : (
                    t.term
                  )}
                  <span
                    ...
                    className="ml-1 opacity-0 group-hover/term:opacity-50 hover:!opacity-100 transition-opacity shrink-0 cursor-pointer"
                  >
                    <svg
                      width="8"
                      height="8"
